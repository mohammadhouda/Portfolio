...
export default function ProjectVisual({
    ...
    return (
      <div className="relative aspect-[3/2] w-full overflow-hidden bg-paper-sunk">
        <div className="absolute inset-0 flex flex-col justify-between p-7 md:p-9">
          <span className="t-meta text-ink-4">{project.tag}</span>

          <div className="flex flex-wrap gap-x-3 gap-y-1.5">
            {project.stack.slice(0, 7).map((tech) => (
              <span
                key={tech}
                className="font-display text-[clamp(1.1rem,2.4vw,1.9rem)] leading-tight text-ink-3"
              >
                {tech}
              </span>
            ))}
  ...
  return (
    <Parallax amount={8} className="relative aspect-[3/2] w-full bg-paper-sunk">
      <Image
        src={src}
